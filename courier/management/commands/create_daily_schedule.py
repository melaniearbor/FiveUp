...
def create_schedule():
    ...
    day_start = make_aware(
        datetime.datetime(today.year, today.month, today.day, 6, 0, 0, 000000),
...
def record_user_send_times(group):
    ...
    for user in group:
        if user.receiving_messages is True & user.is_active is True:
            user_schedule = group_schedule[0 : int(user.how_many_messages)]
            for time in user_schedule:
                user_send_time = UserSendTime.objects.create(
                    scheduled_time=time, user=user
                )
